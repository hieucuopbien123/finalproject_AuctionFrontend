...
const SealedBidV2ActionHeader = ({data, setRefetchTrigger}) => {
  const { hours, minutes, seconds, ended } = useSealedBidV2Timer({data, callbackWhenEnd: () => setRefetchTrigger(p => !p)});
    ...
    <>        
      <Box onClick={() => {
        ...
        <Box display={"flex"} gap="10px" alignItems={"center"}>
          ...
          <Typography fontFamily={"Poppins"}>Sealed bid auction V2</Typography>
          {
            data.status == 1 ? 
            <Box px={1} py={0.5} 
              sx={{
...
export default SealedBidV2ActionHeader;