...
const AuctionInfo = ({data, currentStep}) => {
  ...
  return (
    <Box display="flex" flexDirection={"column"} gap="15px">
      ...
      <Box display="flex" justifyContent="space-between">
        <Typography color={theme.palette.primary.main}>MINIMUM PRICE</Typography>
        <Typography>{formatEther(data.minimumPrice)} {getTokenName("0x0000000000000000000000000000000000000000").symbol}</Typography>
      </Box>      
      <Box display="flex" justifyContent="space-between">
        <Typography color={theme.palette.primary.main}>STEP</Typography>
        <Typography>{currentStep} / {data.numberOfStep}</Typography>
      </Box> 
      <Box display="flex" justifyContent="space-between">
        <Typography color={theme.palette.primary.main}>START AT</Typography>
        <Typography>{(new Date(data.startTime*1000)).toLocaleString()}</Typography>
      </Box>
      <Box display="flex" justifyContent="space-between">
        <Typography color={theme.palette.primary.main}>CREATED AT</Typography>
        <Typography>{(new Date(data.timestamp*1000)).toLocaleString()}</Typography>
      </Box>
      <Box display="flex" justifyContent="space-between">
      ...
      <Box display="flex" justifyContent="space-between">
        <Typography color={theme.palette.primary.main}>ONE STEP DURATION</Typography>
        <Typography>{convertSecondsToTime(data.stepDuration)}</Typography>
      </Box>
      <Box display="flex" justifyContent="space-between">
