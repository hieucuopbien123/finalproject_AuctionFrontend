...
const DutchActionHeader = ({data, setRefetchTrigger}) => {
  const { hours, minutes, seconds, ended } = useDutchTimer({data, callbackWhenEnd: () => setRefetchTrigger(p => !p)});
    ...
    <>        
      <Box onClick={() => {
        ...
        <Box display={"flex"} gap="10px" alignItems={"center"}>
          ...
          <Typography fontFamily={"Poppins"}>Dutch auction</Typography>
          {
            data.status == 1 ? 
            <Box px={1} py={0.5} 
              sx={{
...
export default DutchActionHeader;