...
import useUserActivities from "src/hooks/reactquery/useUserActivities";
...
const UserActivity = () => {
  ...
  const { data: trades, error, isError, isLoading, hasNextPage, fetchNextPage, isFetchingNextPage } = useUserActivities();
          ...
          filteredTrades?.map((t,d) => (
            <Box key={t.hash} minWidth="780px">
              ...
              <Box display="flex" gap="20px" alignItems="center" justifyContent={"space-between"}>
                ...
                <Box minWidth="110px">
                  <CustomTooltip text={t.auctionDetail.id}>
                    <Typography sx={{cursor: "pointer"}} onClick={() => {
                      if(t.auctionDetail.id && t.auctionDetail.id != "0x0000000000000000000000000000000000000000")
                      navigate(`/auctiondetail/${t.auctionDetail.id}`)
                    }}>{formatAddress(t.auctionDetail.id)}</Typography>
                  </CustomTooltip>
                  <Typography className="fontSmallSize" color={theme.palette.primary.main}>Auction</Typography>
                </Box>
                <Box minWidth={"70px"}>
