...
import useUserBiddedAuctions from "src/hooks/reactquery/useUserBiddedAuctions";
...
const CollectionList = ({ collectionAddress, gridMode, searchTerms, auctionTypeFilter, account, status }) => {
  ...
  const { data: userBiddedAuctions, isError, error, isLoading, hasNextPage, fetchNextPage, isFetchingNextPage } 
    = useUserBiddedAuctions({
      userAddress, 
      collectionAddress: debouncedCollectionAddress.map(c => c.address),
  ...
  useEffect(() => {
    startTransition(() => {
      setFilteredAuctions(userBiddedAuctions?.pages?.map((page) => page.data)?.flat() ?? []);
    });
  }, [userBiddedAuctions]);
