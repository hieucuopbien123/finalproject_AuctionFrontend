...
const SealedBidAuctionV1Creation = ({ data, writeContract, isLoading, callback, nftAmount }) => {
  ...
  const createNewSealedBidAuctionV1 = async () => {
    ...
    await transferNFT(vickreyParams, AuctionType.Seal1);
    callback();
  }
  ...
  return (
    <Box py={2} px={4} display={"flex"} flexDirection={"column"} gap="10px" height="100%" justifyContent={"space-evenly"} minHeight="320px">
      ...
      <CreateActionButton onClick={createNewSealedBidAuctionV1} isLoading={isLoading}/>
    </Box>
  )
}

export default SealedBidAuctionV1Creation;